...
struct parameter {
  ...
  const char* str_data() const {return data;}
  const double* real_data() const {return reinterpret_cast<double*>(data);}
  int	id;
  long	nr,nc;
  bool	owner; // detrmines if data is delted when object is deleted
  char*	data;
};
...
public:
  ...
  const parameter* argin(int np)const {return in->parm(np);}
  const parameter* argout(int np)const { return out ? out->parm(np) : 0 ;}
  
  command_id id() const {return _id;};
  size_t nargin() const {return in->np();}
  size_t nargout() const {return out ? out->np() : 0;}

  int call(int fd) {in->write(fd); out=new arg_sendrec(fd); return 0;}
  void ret(int fd) {out->write(fd);}
...
inline void ret_warning(command& com,const char* warn)
{
  char* warn_text=new char[strlen(warn)+1];
  strcpy(warn_text,warn);
  com.argout(0,ocpl::warn,strlen(warn_text)+1,1,warn_text,true);
}
